...
class HourglassModel(object):
    ...
    def upsample_nn(self, x, ratio):
        ...
        return tf.image.resize_images(x, [h * ratio, w * ratio])
    ...
    def inception(self, x, config, is_training=False):
        ...
        conv0 = tf.layers.conv2d(x, out_layers, kernel_size, stride)
        conv0 = tf.layers.batch_normalization(conv0, center=False, scale=False)
        ...
        for i in range(1, len(config)):
            # with tf.variable_scope('inception_' + str(i)):
            filt = config[i][0]
            ...
            # print ("out_b")
            # print (out_b)
            conv1 = tf.layers.conv2d(x, out_a, 1, stride)  # kernel size = 1
            conv1 = tf.layers.batch_normalization(conv1, center=False, scale=False)
            conv1 = tf.nn.relu(conv1)

            conv2 = tf.layers.conv2d(conv1, out_b, filt, stride, padding='same')  # kernel size = filt
            conv2 = tf.layers.batch_normalization(conv2, center=False, scale=False)
    ...
    def Channels1(self, x, is_training=False):
        ...
        with tf.variable_scope('0/1'):
            with tf.variable_scope('0'):
                pool1b = tf.layers.average_pooling2d(x, 2, 2)
            with tf.variable_scope('1'):
    ...
    def Channels3(self, x, is_training=False):
        with tf.variable_scope('0/0'):
            with tf.variable_scope('0'):
                pool1b = tf.layers.max_pooling2d(x, 2, 2)
            with tf.variable_scope('1'):
    ...
    def fridaymodel(self, input, is_training):
        ...
        with tf.name_scope('module'):
            with tf.variable_scope('0'):
                conv1a = tf.layers.conv2d(input, out_layers_1a, kernel_size_1a, padding='same', activation=None,
                                          name='conv2d')

            with tf.variable_scope('1'):
                conv1a_bn = tf.layers.batch_normalization(conv1a)
            ...
            with tf.variable_scope('4'):
                conv3a = tf.layers.conv2d(conv2a, out_layers_3a, kernel_size_3a, stride_3a, padding='same',
                                          activation=None, name='conv2d')
            # post-processing
            conv3a = tf.squeeze(conv3a)  # (1, 384, 512, 1)
            conv3a = tf.exp(conv3a)
            pred_inv_depth = tf.div(1.0, conv3a)
            pred_inv_depth = tf.div(pred_inv_depth, tf.reduce_max(pred_inv_depth))
